...
int main() {
  ...
  auto speed = ConfigManager::Instance().Get<double>("robot.max_speed");
  std::cout << "The max speed of this robot is: " << speed << std::endl;
  ...
  robot.onBatteryChange([](const Battery &battery) {
    std::cout << "Battery: " << battery.getBatteryStatus() << std::endl;
  });
